...
export function AdminDisputesTable({ disputes }: { disputes: DisputeRow[] }) {
  ...
  async function mutateDispute(key: string, payload: Record<string, unknown>, successMessage: string) {
    setPendingKey(key);
    try {
          ...
          {disputes.map((dispute) => {
            ...
            return (
              <tr key={dispute.id} className="border-b align-top hover:bg-muted/30">
                ...
                <td className="px-4 py-3 text-muted-foreground">{dispute.assignedAdminName ?? "Unassigned"}</td>
                <td className="px-4 py-3 text-xs text-muted-foreground">{dispute.resolution ?? "Pending"}</td>
                <td className="px-4 py-3 text-xs text-muted-foreground">{dispute.createdAtLabel}</td>
                <td className="px-4 py-3">
                  <div className="min-w-[240px] space-y-2">
                    {!dispute.assignedAdminName ? (
                      <Button
                        ...
                        variant="outline"
                        disabled={isBusy}
                        onClick={() => void mutateDispute(assignKey, { disputeId: dispute.id, assignToMe: true }, "Dispute assigned.")}
                      >
                        Assign to me
                      </Button>
                    ) : null}
                      ...
                      <div className="space-y-2">
                        ...
                        <Button
                          ...
                          onClick={() =>
                            void mutateDispute(
                              resolveKey,
                              { disputeId: dispute.id, status: "resolved", resolution: resolutionValue.trim() },
                              "Dispute resolved."
                            )
                          }
                        >
