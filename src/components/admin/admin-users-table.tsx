...
export function AdminUsersTable({ users }: { users: UserRow[] }) {
  ...
  async function changeRole(user: UserRow, nextRole: string) {
    ...
    try {
      await mutateUser(`role-${user.id}`, { userId: user.id, role: nextRole }, `${user.fullName}'s role updated.`);
    } catch {
                ...
                <td className="px-4 py-3">
                  {user.status === "active" ? (
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            Deactivate {user.fullName}? This will suspend account access until an admin reactivates the user.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                          ...
                          <AlertDialogAction
                            ...
                            onClick={() => void mutateUser(deactivateKey, { userId: user.id, status: "suspended" }, `${user.fullName} deactivated.`)}
                            disabled={isBusy}
                          >
                            Deactivate
                          </AlertDialogAction>
                    ...
                    <Button
                      ...
                      variant="outline"
                      disabled={isBusy}
                      onClick={() => void mutateUser(reactivateKey, { userId: user.id, status: "active" }, `${user.fullName} reactivated.`)}
                    >
