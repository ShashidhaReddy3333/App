...
export default async function OperationsPage() {
  ...
  const summaryCards = [
    { title: "Active sessions", value: snapshot.summary.activeSessions.toString(), numValue: snapshot.summary.activeSessions },
    { title: "Pending invites", value: snapshot.summary.pendingInvites.toString(), numValue: snapshot.summary.pendingInvites },
    { title: "Queued notifications", value: snapshot.summary.queuedNotifications.toString(), numValue: snapshot.summary.queuedNotifications },
    { title: "Failed notifications", value: snapshot.summary.failedNotifications.toString(), numValue: snapshot.summary.failedNotifications },
    { title: "Expired reset tokens", value: snapshot.summary.expiredResetTokens.toString(), numValue: snapshot.summary.expiredResetTokens },
    { title: "Stale reservations", value: snapshot.summary.staleReservations.toString(), numValue: snapshot.summary.staleReservations },
    { title: "Runtime errors", value: snapshot.summary.runtimeErrors.toString(), numValue: snapshot.summary.runtimeErrors },
    { title: "Runtime warnings", value: snapshot.summary.runtimeWarnings.toString(), numValue: snapshot.summary.runtimeWarnings }
  ];
                ...
                <div className="flex items-center justify-between">
                  ...
                  {variant !== "default" && (
                    <Badge variant={variant === "success" ? "success" : variant}>
                      {variant === "success" ? "clear" : variant === "destructive" ? "alert" : "check"}
                    </Badge>
                  )}
                </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <div className="space-y-4 animate-fade-in-up stagger-3">
          ...
          {failedNotifications.length === 0 ? (
            <EmptyState icon="inbox" title="No failed notifications" description="Failed delivery attempts will appear here for owner review and retry." />
          ) : (
        ...
        <div className="space-y-6 animate-fade-in-up stagger-4">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Badge variant={snapshot.runtimeIssues.length > 0 ? "warning" : "outline"}>{snapshot.runtimeIssues.length}</Badge>
            </div>
                  ...
                  <Card key={`${issue.severity}-${issue.key}`} className={`gradient-panel ${
                    ...
                    <CardHeader className="pb-2">
                      <div className="flex items-center justify-between">
                        ...
                        <Badge variant={issue.severity === "error" ? "destructive" : issue.severity === "warning" ? "warning" : "outline"}>
                          {issue.severity}
                        </Badge>
                      </div>
                    </CardHeader>
