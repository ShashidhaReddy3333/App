...
function getPoBadgeVariant(status: string): "success" | "warning" | "destructive" | "default" | "secondary" {
  ...
  if (status === "draft" || status === "ordered") return "warning";
  if (status === "cancelled" || status === "rejected") return "destructive";
  if (status === "sent" || status === "accepted" || status === "partially_received") return "default";
  return "secondary";
}
            ...
            <Card key={purchaseOrder.id} className="gradient-panel">
              <CardHeader className="pb-3">
                <CardTitle className="flex items-center justify-between gap-3">
                  ...
                  <Badge variant={getPoBadgeVariant(purchaseOrder.status)}>
              ...
              <CardContent className="space-y-4">
                <div className="grid gap-3 text-sm md:grid-cols-2">
                  <div>
                    <span className="text-xs uppercase tracking-wide text-muted-foreground/70">Supplier</span>
                  ...
                  <div>
                    ...
                    <div className="font-semibold">${Number(purchaseOrder.totalCost).toFixed(2)}</div>
                  </div>
                </div>
                <div className="space-y-2 text-sm">
                  {purchaseOrder.items.map((item) => (
                    <div key={item.id} className="flex justify-between rounded-xl border border-border/60 bg-white/60 p-3 transition-colors hover:bg-muted/50">
