...
export default async function CustomerDashboardPage() {
  ...
  const completedCount = analytics.statusBreakdown.find((entry) => entry.status === "completed")?.count ?? 0;
  const completionRate = analytics.totalOrders > 0 ? (completedCount / analytics.totalOrders) * 100 : 0;
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {metricCards.map((metric, index) => {
            ...
            return (
              <Card key={metric.label} className="rounded-xl border border-border bg-white shadow-sm">
          ...
          <CardContent className="space-y-0">
            {analytics.topProducts.length === 0 ? (
              <div className="flex flex-col items-start gap-3 py-2">
                <p className="text-sm text-muted-foreground">Your most ordered products will appear after completed checkouts.</p>
              ...
              analytics.topProducts.map((product, index) => (
                <div key={product.productId} className="flex items-center justify-between gap-3 border-b border-border py-3 last:border-b-0">
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">{product.timesOrdered} times ordered</p>
                    </div>
                  </div>
