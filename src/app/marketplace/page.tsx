...
export default async function MarketplacePage({
  ...
  const paginationBasePath = paginationParams.toString() ? `/marketplace?${paginationParams.toString()}` : "/marketplace";
                ...
                <Card className="h-full cursor-pointer transition-shadow hover:shadow-md">
                  <CardContent className="p-4">
                    ...
                    <div className="mb-1 flex items-start justify-between gap-2">
                      <h3 className="text-sm font-semibold leading-tight">{profile.business.businessName}</h3>
                      {profile.isFeatured ? (
                    ...
                    {profile.tagline ? (
                      <p className="mb-2 line-clamp-2 text-xs text-muted-foreground">{profile.tagline}</p>
                    ) : null}
